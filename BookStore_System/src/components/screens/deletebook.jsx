...
export const DeleteBook = () => {
    ...
    const deleteBook = (title) => {
        axios.delete(`http://localhost:3000/delete/${encodeURIComponent(title)}`)
            .then(() => {
                    ...
                    <ul>
                        {books.map(book => (
                            <li key={book._id} className="flex justify-between items-center bg-white p-4 rounded-lg shadow mb-2 dark:bg-gray-900">
                                ...
                                <button onClick={() => deleteBook(book.title)} className="text-white bg-red-500 hover:bg-red-600 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2 text-center">Delete</button>
                            </li>
                        ))}
                    </ul>
                </div>
